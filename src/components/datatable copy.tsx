...
const CustomDatatable = ({
  ...
  const Sorter: React.FC<SorterProps> = ({ rel }) => {
    ...
    const sortHandler = (sortBy: string, isDate: boolean) => {
      const sortFunction = sortedBy ? updateSortBy : updateUnSortBy;
      sortFunction(sortBy, isDate);
  ...
  const updateSortBy = (sortBy: string, isDate = false): void => {
    setBodyState((prev) => {
      return [
        ...prev.sort((a, b) => {
  ...
  const updateUnSortBy = (sortBy: any, isDate = false): void => {
    setBodyState((prev) => [
      ...prev.sort((a, b) => {
        const compareValue = isDate
